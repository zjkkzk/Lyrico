...
当前协议中，manifest 只声明身份、版本、入口、能力和 `configFields`。`requiredHostApis`、`metadataFields`、`returnedFields` 以及字段写入策略声明已移除；插件运行结果通过 `fields` 返回标准元数据，通过 `internal` 返回插件私有上下文。
...
| 组件 | 职责 |
|------|------|
| `PluginManifest` | 插件声明数据模型，定义插件元数据、能力、配置 |
