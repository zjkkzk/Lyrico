...
**"标签匹配"** 使用已启用的元数据源写入标题、艺术家、专辑等元数据字段。歌词与封面分别通过独立的 **"批量匹配歌词"**、**"批量匹配封面"** 任务处理。

在批量菜单中选择对应任务，确认并发数和写入配置后开始。三类任务分别使用插件管理中对应类型的启用状态与优先级；仅提供歌词的源可直接返回候选，封面候选会按本地歌曲信息评分选择。
...
计算使用的目标响度取自 `设置` → `扫描设置` → `回放增益参考响度`（默认 -18 LUFS）。

---

## 导出歌词和封面

**“导出歌词”** 可将歌词导出到统一的指定文件夹，或分别保存到每首音频所在目录。**“导出封面”** 会将封面导出到指定目录。
...
4. 导出歌词时，选择 **“指定文件夹”** 或 **“音频同目录”**；选择指定文件夹或导出封面时，再在系统文件选择器中选择目标目录。
5. Lyrico 开始导出。无歌词/封面的歌曲会跳过。

导出规则：
- TTML 歌词导出为 `.ttml`；其他歌词导出为 `.lrc`。
- 选择“音频同目录”时，每首歌词会分别写入对应音频文件所在的目录，所选音频可以来自不同目录。
- 封面以原音频文件名为基础导出为 `.jpg`。
